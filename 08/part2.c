...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/12/09 02:25:49 by dchernik          #+#    #+#             */
/*   Updated: 2024/12/09 04:22:32 by dchernik         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int is_points_within_bounds(t_point p1, t_point p2, int w, int h)
{
    int res;
...
int construct_line(t_point p1, t_point p2, int w, int h)
{
    int nx;
    int ny;
    int dx;
    ...
    if (p1.X > p2.X && p1.Y < p2.Y) /* First quadrant */
    {
        nx = p1.X;
        ny = p1.Y;
        while (is_point_within_bounds((t_point){.X=nx, .Y=ny}, w, h))
        {
            m[ny][nx] = '#';
            nx += dx;
            ny -= dy;
    ...
    for (int ki = 0; ki < freq_cnt; ki++)
    {
        ...
        { 
            for (int aj = 0; aj < anum; aj++)
            {
                if (ai != aj)
                {
                    construct_line(f[ki].ants[ai], f[ki].ants[aj], width, height);
                }
            } // for (int aj = 0; aj < anum; aj++)
        } // for (int ai = 0; ai < anum; ai++)
    } // for (int ki = 0; ki < freq_cnt; ki++)
